...
def lcmm(l):  # lcm of large set of numbers
    return reduce(lambda x, y: lcm(x, y), l)
   
def matmult(a,b):  # multiply corresponding values 
    zip_b = zip(*b)  # transpose matrix
...
def answer(m):
	...
	order = abso + nabso # new matrix

	# prepare limiting matrix
	limM = []
	n = 0
	for i in abso:
            limM.append(m[i])  # create identity matrix
            limM[n][n] = 1 
            n += 1
	for i in nabso:  # create matrix edges
            temp, temp2 = [], []
            for n in order:
                temp.append(m[i][n])
	...
	for u in range(0, len(Q)):  # generate identity matrix
            temp = [0] * len(Q)
            temp[u] = 1
            I.append(temp)
	...
	F = getMatrixInverse(IQ)  # Invert matrix
	
	FR = matmult(F,R)  # Multiply corresponding
	...
	lcm1 = lcmm([i[1] for i in alm])  # get lcm of denom
